...
void CQueue_3DM_GX3::Clear()
{
	m_iHead = m_iTail =0;
	memset(buff,0,BUFF_SIZE);
}
CQueue_3DM_GX3::CQueue_3DM_GX3()
{
	Clear();
}
int CQueue_3DM_GX3::GetSize()
{
	return (m_iHead - m_iTail + BUFF_SIZE) % BUFF_SIZE;
}
BOOL CQueue_3DM_GX3::PutByte(BYTE b)
{
...
BOOL CQueue_3DM_GX3::GetByte(BYTE *pb)
{
...
//--- 클래스 생성자
CCommThread_3DM_GX3::CCommThread_3DM_GX3()
{
...
BOOL CCommThread_3DM_GX3::OpenPort(CString strPortName, 
...
//			AfxMessageBox("fail Port open");
	...
	m_hThreadWatchComm = CreateThread( NULL, 0, 
									   (LPTHREAD_START_ROUTINE)ThreadWatchComm_3DM_GX3, 
									   this, 0, &dwThreadID);
...
// 포트를 닫는다.
void CCommThread_3DM_GX3::ClosePort()
{
	//--> 연결되지 않았음.
	m_bConnected = FALSE;
...
// 실제로 쓰여진 Byte수를 리턴한다.
DWORD CCommThread_3DM_GX3::WriteComm(BYTE *pBuff, DWORD nToWrite)
{
...
// 실제로 읽혀진 Byte수를 리턴한다.
DWORD CCommThread_3DM_GX3::ReadComm(BYTE *pBuff, DWORD nToRead)
{
...
DWORD	ThreadWatchComm_3DM_GX3(CCommThread_3DM_GX3* pComm)
{
   DWORD           dwEvent;
   OVERLAPPED      os;
   BOOL            bOk = TRUE;
...
//		AfxMessageBox("Error while creating ThreadWatchComm, " + pComm->m_sPortName);
   ...
   while (pComm ->m_bConnected)//포트가 연결되면 무한 루프에 들어감
   {
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
        {
			...
			((interface_3DM_GX3*)pComm->Parent)->GetData();
